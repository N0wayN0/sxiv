...
void show_top_bar(win_t *win)
{
	...
	fprintf(stderr,"mojafunkcja w window.c:387");
	//gc = XCreateGC(e->dpy,win->xwin, 0, None);
	//XSetForeground(e->dpy, gc, win->red.pixel);
	//XFillRectangle(win->env.dpy, win->buf.pm, gc, 100, 100, 200, 200);
	...
	win_draw_text(win, d, &win->red, 300, 20, napis, strlen(napis), w);
	napis = "X";
	win_draw_text(win, d, &win->red, win->w-10, 20, napis, strlen(napis), w);
	//XftDrawDestroy(d);
}
